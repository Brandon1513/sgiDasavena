...
═══════════════════════════════════════════ --}}
    <div class="sgi-root min-h-screen relative bg-fixed bg-center bg-cover"
        ...
        {{-- Capa de luz suave sobre el fondo --}}
        ...
        <div class="relative z-10 min-h-screen flex items-start justify-center py-10 px-4 sm:px-6 lg:px-8">
            ...
            ═══════════════════════════════ --}}
                <div class="crystal-hero dc-header-face  tilt-card sgi-reveal" style="--ri:1">
                    ...
                    <img src="/images/Valentia .png" alt="Valentia " class="fixed  left-6  object-contain z-10 select-none " style="mix-blend-mode: multiply; width: 300px; height: auto 
                    ; top:5%; margin-left:850px; ">
                ...
                {{-- ═══════════════════════════════
                 TARJETAS DE ESTADO — 3D FLIP 
            ═══════════════════════════════ --}}
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sgi-reveal" style="--ri:2">
                    @foreach([
                    ['label'=>'Pendientes', 'val'=>$pendientes??'—', 'cls'=>'fc-indigo', 'desc'=>'Esperando revisión del jefe'],
                    ['label'=>'Aprobado jefe', 'val'=>$aprobadoJefe??'—', 'cls'=>'fc-amber', 'desc'=>'En proceso de gestión SGI'],
                    ['label'=>'Atendidas', 'val'=>$atendidas??'—', 'cls'=>'fc-emerald', 'desc'=>'Formato actualizado con éxito'],
                    ['label'=>'Rechazadas', 'val'=>$rechazadas??'—', 'cls'=>'fc-rose', 'desc'=>'Requieren revisión y reenvío'],
                    ] as $s)
                    <article class="flip-card">
                        <div class="flip-card-inner">
                            <div class="flip-front crystal-card {{ $s['cls'] }} border border-white/50 shadow-lg flex flex-col items-center justify-center gap-3 p-6">
                                <div class="crystal-shine"></div>
                                <span class="fc-num text-5xl sm:text-6xl font-black">{{ $s['val'] }}</span>
                                <span class="fc-label text-xs sm:text-sm tracking-wider font-semibold">{{ $s['label'] }}</span>
                            </div>
                            <div class="flip-back crystal-card {{ $s['cls'] }}-back border border-white/50 shadow-lg flex flex-col items-center justify-center gap-3 p-6">
                                <div class="crystal-shine"></div>
                                <p class="fc-back-desc text-sm sm:text-base leading-relaxed px-2 font-medium">{{ $s['desc'] }}</p>
                                <span class="inline-flex items-center justify-center rounded-lg bg-white/30 px-4 py-2 text-xs font-bold uppercase tracking-widest hover:bg-white/40 transition-all">
                                    Ver más
                                </span>
                            </div>
                        </div>
...
══════════════════════════════════════ */
        ...
        .sgi-toast-live {
            ...
            animation: toast-live 1.6s ease-in-out infinite;
            flex-shrink: 0;
        }
        ...
        @keyframes toast-live {
            ...
            50% {
                box-shadow: 0 0 0 6px rgba(16, 185, 129, .05)
            }
        }
        ...
        .sgi-toast-bar {
            height: 3px;
            width: 100%;
            background: linear-gradient(90deg, #818cf8, #4f46e5, #7c3aed);
        ...
        .sgi-pulse-ring {
            ...
            border-radius: 50%;
            background: #D6A644;
            box-shadow: 0 0 0 3px rgba(100, 500, 68, .3);
            animation: toast-live 1.6s ease-in-out infinite;
            flex-shrink: 0;
        }
...
══════════════════════════════════════ */
        ...
        .orb-shadow {
            ...
            filter: blur(12px);
            opacity: .45;
            animation: orb-shadow-float 5s cubic-bezier(.4, 0, .6, 1) infinite;
        }
        ...
        /* ── EFECTO HOVER ── */
        .orb-wrap:hover .orb {
            animation: orb-pulse-intense .6s ease-out;
            filter: drop-shadow(0 20px 60px rgba(99, 102, 241, .5));
        }
        ...
        /* ══════════════════════════════════════
    FLIP CARDS DE ESTADO — DISEÑO MEJORADO
══════════════════════════════════════ */
        .flip-card {
            perspective: 1200px;
            height: 140px;
        ...
        .flip-card:hover {
            filter: drop-shadow(0 12px 32px rgba(99, 102, 241, .25));
        }
        ...
        .flip-back {
            ...
            box-shadow:
                0 1px 0 0 rgba(255, 255, 255, 1.9) inset,
                0 8px 32px rgba(99, 102, 2, .12),
                0 2px 8px rgba(0, 0, 0, .06);
        ...
        .fc-indigo {
            --grad-start: #4f46e5;
            --grad-end: #6366f1;
            background: linear-gradient(135deg, rgba(238, 242, 255, .85), rgba(224, 231, 255, .7));
            border-color: rgba(165, 180, 252, .6);
        }

        .fc-indigo-back {
            background: linear-gradient(135deg, rgba(224, 231, 255, .95), rgba(209, 222, 255, .8));
        ...
        .fc-amber {
            --grad-start: #d97706;
            --grad-end: #f59e0b;
            background: linear-gradient(135deg, rgba(255, 251, 235, .85), rgba(254, 243, 191, .7));
            border-color: rgba(253, 211, 77, .5);
        }

        .fc-amber-back {
            background: linear-gradient(135deg, rgba(254, 243, 199, .95), rgba(255, 236, 179, .8));
            border-color: rgba(253, 190, 75, .4);
        }
        ...
        .fc-emerald {
            --grad-start: #059669;
            --grad-end: #10b981;
            background: linear-gradient(135deg, rgba(236, 253, 245, .85), rgba(209, 250, 229, .7));
            border-color: rgba(110, 231, 183, .5);
        }

        .fc-emerald-back {
            background: linear-gradient(135deg, rgba(209, 250, 229, .95), rgba(167, 243, 208, .8));
        ...
        .fc-rose {
            --grad-start: #be123c;
            --grad-end: #f43f5e;
            background: linear-gradient(135deg, rgba(255, 241, 242, .85), rgba(255, 228, 230, .7));
            border-color: rgba(253, 164, 175, .5);
        }

        .fc-rose-back {
            background: linear-gradient(135deg, rgba(255, 228, 230, .95), rgba(254, 205, 211, .8));
...
══════════════════════════════════════ */
        ...
        .acc3d-teal:hover {
            ...
            box-shadow: 0 8px 32px rgba(20, 184, 166, .12);
        }
        ...
        .acc3d-head::-webkit-details-marker {
            display: none;
        }
...
══════════════════════════════════════ */
        ...
        .info3d-bar {
            height: 3px;
            background: linear-gradient(90deg, rgba(99, 102, 241, .5), rgba(124, 58, 237, .3));
        }

        .info3d-sky .info3d-bar {
            background: linear-gradient(90deg, rgba(14, 165, 233, .5), rgba(56, 189, 248, .3));
        }

        .info3d-emerald .info3d-bar {
            background: linear-gradient(90deg, rgba(5, 150, 105, .5), rgba(16, 185, 129, .3));
        }
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        padding: 12,
                        cornerRadius: 12,
                        boxShadow: '0 4px 20px rgba(79,70,229,.15)',
                        callbacks: {
