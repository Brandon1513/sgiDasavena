                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white border">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase border-b border-gray-200">Nombre</th>
                                    <th class="px-4 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase border-b border-gray-200">Email</th>
                            ...
                            <tbody>
                                @foreach($users as $user)
                                    <tr>
                                        <td class="px-4 py-4 border-b border-gray-200 whitespace-nowrap">{{ $user->name }}</td>
                                        <td class="px-4 py-4 border-b border-gray-200 whitespace-nowrap">{{ $user->email }}</td>
                                        <td class="px-4 py-4 border-b border-gray-200 whitespace-nowrap">{{ $user->activo ? 'Activo' : 'Inactivo' }}</td>
                                        <td class="px-4 py-4 border-b border-gray-200 whitespace-nowrap">
                                            <!-- Botón Editar -->
                                            <a href="{{ route('usuarios.edit', $user->id) }}" class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-white uppercase transition bg-yellow-500 border border-transparent rounded-md hover:bg-yellow-600 active:bg-yellow-700 focus:outline-none focus:border-yellow-700 focus:ring focus:ring-yellow-200 disabled:opacity-25">
                                                Editar
                                            </a>
