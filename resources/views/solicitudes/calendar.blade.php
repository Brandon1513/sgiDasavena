<x-app-layout>
    ...
    <div class="py-12 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 min-h-screen">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="group relative bg-white backdrop-blur-xl border border-red-200/50 p-6 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
                ...
                <p class="text-xs text-red-500/70 mt-2 font-medium">Menor a 30 días (incluye vencidos)</p>
            </div>
            ...
            <div class="p-8 border-b border-gray-200/50 bg-gradient-to-r from-white via-blue-50/30 to-white">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                        ...
                        <input id="historicos" type="checkbox" class="rounded border-gray-300">
                        <span class="text-sm font-semibold text-gray-700">Ver históricos</span>
                    </div>
  ...
  const historicosEl = document.getElementById('historicos');
  ...
  // ✅ clave YYYY-MM-DD en hora local (sin UTC)
  ...
  function loadMonth(){
    const params = new URLSearchParams({
      ...
      historicos: historicosEl.checked ? '1' : '0',
    });
      ...
      .then(data => {
        ...
        for(let i=0;i<42;i++){
          ...
          events.slice(0,2).forEach(e => {
            ...
            // ✅ link (si viene)
            const linkShow = e.url_documento || e.url_version || null;

            html += `
              <div class="${color} p-3 rounded-lg text-xs hover:shadow-lg transition-all duration-300">
                <div class="flex justify-between gap-2 items-center">
                  <strong class="text-gray-900 font-bold">${escapeHtml(e.codigo ?? '')}</strong>
                  <span class="text-[10px] text-gray-600 bg-white/80 backdrop-blur-sm px-2 py-1 rounded-md font-semibold">${tipoTxt}</span>
                </div>
                <div class="text-gray-700 truncate text-[11px] mt-1 font-medium">${escapeHtml(e.nombre ?? '')}</div>
                <div class="flex items-center justify-between mt-2">
                  <div class="font-bold text-gray-800">⏱ ${e.days_left ?? ''} días</div>
                  ${linkShow ? `
                    <a href="${linkShow}" class="text-[11px] font-bold text-cyan-700 hover:underline">
                      Ver
                    </a>
                  ` : ``}
                </div>
        ...
        (data.list || []).forEach(row => {
          ...
          const b = badge(Number(row.days_left ?? 999999));
          const linkShow = row.url_documento || row.url_version || '#';
          ...
          listBody.insertAdjacentHTML('beforeend', `
            <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-cyan-50 transition-all duration-300">
              ...
              <td class="p-4 text-gray-700 font-medium">${escapeHtml(row.fecha_vencimiento ?? '')}</td>
              <td class="p-4 text-gray-700 font-medium">${tipoTxt}</td>
              <td class="p-4 font-bold text-gray-900">${escapeHtml(row.codigo_documento ?? '')}</td>
              <td class="p-4 text-gray-700 font-medium">${escapeHtml(row.nombre_documento ?? '')}</td>
              <td class="p-4 text-gray-600">${escapeHtml(row.tipo_documento ?? '')}</td>
              <td class="p-4 text-gray-600">${escapeHtml(row.formato_el_pa ?? '')}</td>
              <td class="p-4 text-gray-600">${escapeHtml(row.folio_version ?? '')}</td>
              <td class="p-4 text-gray-600 truncate max-w-[240px]">${escapeHtml(row.lugar_almacenamiento ?? '')}</td>
              <td class="p-4">
        ...
        // ✅ si filtras por estado, mandarte a lista
